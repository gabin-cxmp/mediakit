...
dom.logoUpload.addEventListener('change', () => {
    dom.fileNameDisplay.textContent = dom.logoUpload.files[0]?.name || '(No file chosen)';
  });

dom.pictureUpload.addEventListener('change', () => {
    dom.fileNamePictures.textContent = dom.pictureUpload.files[0]?.name || '(No file chosen)';
});
...
const downloadAllImages = (imageData) => {
    ...
    zip.generateAsync({ type: 'blob' }).then(content => saveAs(content, 'My-Customized-Logo-PC25-Kit.zip'));
};
...
const downloadAllPictures = (imageData) => {
  ...
  zip.generateAsync({ type: 'blob' }).then(content => saveAs(content, 'My-Customized-Picture-PC25-Kit.zip'));
};
...
dom.genericDownloadAll.onclick = async () => {
    ...
    saveAs(await zip.generateAsync({ type: 'blob' }), 'My-Generic-PC25-Kit.zip');
};
...
dom.submitButtonPictures.addEventListener('click', async () => {
  if (!croppieInstance) return alert('Please upload and crop an image first!');
  
  const standNumber = dom.standNumberPictures.value.trim();
  if (!standNumber) return alert('Please enter a stand number!');
  
  const standLetter = dom.standLetterPictures.value.trim();
  if (!standLetter) return alert('Please enter a stand letter!');
